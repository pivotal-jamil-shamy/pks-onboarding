...
ROLE_OR_CLUSTER_ROLE_NAME="view"
...
for cluster in $LIST_OF_CLUSTERS; do
  if [[ -f pks-onboarding-repo/pks/$PKS_ENV/$cluster/pksClusterConfig.yml ]]; then
    ...
    PKS_CLUSTER_READONLY=$(cat pks-onboarding-repo/pks/$PKS_ENV/$cluster/pksClusterConfig.yml | yaml2json | jq -r '.pks_cluster_readonly_ldap_group[]')
    ...
    if is_cluster_exists $PKS_CLUSTER_NAME; then
      ...
      for group in $PKS_CLUSTER_READONLY; do
